...
class SieRegisterSubject(models.Model):
    ...
    student_id = fields.Many2one(
        ...
        required=True,
        domain="[('enrollment_ids', '=', enrollment_id)]",
        ondelete='restrict'
    )
    ...
    def _compute_enrollment(self):
        for record in self:
            if record.course_id:
                record.enrollment_id = record.env['sie.enrollment'].search([('name', '=', record.course_id.name)]).id
