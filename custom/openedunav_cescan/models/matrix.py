...
class SieMatrix(models.Model):
    ...
    name = fields.Char(string='Nombre', requeried=True, store=True)
    display_name = fields.Char(string=u'Descripción', compute='_compute_display_name', store=True)
    parameter_ids = fields.One2many('sie.matrix.parameter', inverse_name='matrix_id',
                                    string='Parameters', store=True, ondelete='cascade')
    promotion_course = fields.Many2one('sie.promotion.course', string=u'Promoción del Curso', ondelete='restrict')
    year = fields.Selection([(num, str(num)) for num in range(2010, datetime.datetime.now().year + 1)], string=u'Año')
    min_aprovechamiento = fields.Integer(string="Min score Aprovechamiento ", default=14)
    min_integrador = fields.Integer(string="Min score Integrador ", default=14)
    min_trabajo = fields.Integer(string="Min score Trabajo ", default=14)
    min_aprendizaje = fields.Integer(string="Min score Aprendizaje ", default=14)
    min_productividad = fields.Integer(string="Min score Productividad", default=14)
    min_actitud = fields.Integer(string="Min score Profesional", default=14)
    minimum_score = fields.Integer(string='Overall Average to Pass', default=15)
    ...
    def copy(self, default=None):
        for record in self:
            ...
            for parameter in parameter_ids:
                ...
                SieMatrix._create_children(self, parameter, new_parameter)
        return new_matrix
