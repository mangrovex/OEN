...
class SieMatrixParameter(models.Model):
    ...
    name = fields.Char(string=u'Codigo', compute='_compute_name')
    param_name = fields.Many2one('sie.param.name', string=u'Nombre del Parámetro', required=True, ondelete='restrict')
    coefficient = fields.Float(string='Coeficiente', digits=dp.get_precision('Coefficient'),
                               required=True, default=0.1)
    parent_id = fields.Many2one('sie.matrix.parameter', string='Padre', ondelete='cascade', index=True)
    child_ids = fields.One2many('sie.matrix.parameter', 'parent_id', string=u'Parametros hijos')
    matrix_id = fields.Many2one('sie.matrix', string=u'Parámetros de evaluación',
                                ondelete='cascade')
    course_ref = fields.Char(string='Curso ref', compute='_compute_course_ref', store=True)
    parent_ref = fields.Char(string='Nombre del padre', compute='_compute_parent_ref', store=True)
    last_child = fields.Boolean(string=u'Último hijo?', compute='_compute_last_child',
                                store=True, default=True)
    total_coefficient = fields.Float(string='Suma Coeficientes', compute='_compute_total_coefficient',
                                     digits=dp.get_precision('Coefficient'),
