...
class SieSubjectUnit(models.Model):
    ...
    name = fields.Char(u'Título', size=128, required=True)
    number = fields.Integer(u'Número', required=True)
    description = fields.Text(u'Descripción')
    parent_id = fields.Many2one('sie.subject.unit', 'Padre', ondelete='cascade')
    child_ids = fields.One2many('sie.subject.unit', 'parent_id', 'Unidades de Aprendizaje')
    subject_id = fields.Many2one('sie.subject', 'Asignatura', ondelete='cascade')
    hours = fields.Integer('No. Horas', default=0)
    total_hours = fields.Integer(compute='_total_hours', string='Total Horas', store=True)
    last_child = fields.Boolean(compute='_compute_last_child', string=u'Último hijo', default=True, store=True)
    subject_ref = fields.Char(string='Asignatura ref', compute='_compute_subject_ref', store=True)
