...
class SieRegisterWorkEnrollment(models.Model):
    ...
    register_work_id = fields.Many2one('sie.register.work', string='Register work', ondelete='restrict',
                                       required=True, domain="[('course_id', '=', course_id)]")
    student_ids = fields.One2many('sie.register.work.student', inverse_name='enrollment_id',
    ...
    def _compute_fullname(self):
        if self.course_id:
            fullname = '%s-%s-%s' % (self.register_work_id.work_name, self.course_id.name, self.group_name)
            self.name = fullname
